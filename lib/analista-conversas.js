...
// O wa-agent retorna { ok, msgs: [{ direction, body, created_at, name, ... }] }.
async function analisarConversa(phone) {
    const data = await waFetch(`/admin/conversas/${phone}?limit=60`);
    const mensagens = data?.msgs || data?.mensagens || data?.messages || (Array.isArray(data) ? data : []);
    if (!mensagens.length) return { ok: false, motivo: 'sem mensagens' };
    const ultima = mensagens[mensagens.length - 1];
    const ultimaMsgAt = ultima?.created_at || ultima?.at || null;
    const leadNome = mensagens.find(m => m.name)?.name || null;
...
// Lista todos os leads/conversas do wa-agent (pra análise retroativa do histórico).
async function listarLeadsWa() {
    const data = await waFetch('/admin/leads');
    return data?.leads || [];
}
...
module.exports = { analisarConversa, analisarMensagens, analisarEsfriadas, listarLeadsWa, aprovarRegra, rejeitarRegra, TIPOS_ERRO };
