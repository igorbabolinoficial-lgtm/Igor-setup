...
const PERFIS = {
  ...
  investidor: {
    ...
    idadeMin: 35,
    idadeMax: 65,
    interesses: [6003397425735, 6003263791608], // Real Estate, Investment
  },
  veraneiro: {
...
export async function criarAdSet({
  ...
  log.info('[meta-ads] AdSet criado', { id: res.id, nome, perfil: perfil || 'custom' });
  return res.id;
}
...
export async function criarCriativo({ nome, imagemHash, titulo, corpo, mensagemInicial, slides }) {
  const waLink = WA_NUMBER
    ? `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(mensagemInicial || 'Olá, tenho interesse em um imóvel!')}`
    : 'https://imobiliariapraiadorosa.com.br';

  const ctaWa = { type: 'WHATSAPP_MESSAGE', value: { app_destination: 'WHATSAPP' } };
...
export async function criarCampanhaCompleta({
  ...
  log.info('[meta-ads] Iniciando campanha completa', { nomeCampanha, adSets: adSets.length });
