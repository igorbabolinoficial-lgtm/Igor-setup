...
const calendar = {
    ...
    /**
     * Atualiza evento existente no Calendar.
     */
    async atualizarEvento(eventId, { titulo, descricao, inicio, fim, localizacao }) {
        const auth = getAuthenticatedClient();
        const cal = google.calendar({ version: 'v3', auth });
        const calendarId = process.env.GOOGLE_CALENDAR_ID || 'primary';
        const patch = {};
        ...
        if (inicio)      patch.start       = { dateTime: inicio, timeZone: 'America/Sao_Paulo' };
        if (fim)         patch.end         = { dateTime: fim,    timeZone: 'America/Sao_Paulo' };
        if (localizacao !== undefined) patch.location = localizacao;
    ...
    /**
     * Cancela (deleta) evento do Calendar. sendUpdates='all' avisa convidados.
    ...
    async cancelarEvento(eventId) {
        ...
        return { ok: true, cancelado: eventId };
    },
